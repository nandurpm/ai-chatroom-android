...
enum class Speaker(val label: String) { USER("You"), CHATGPT("ChatGPT"), GEMINI("Gemini") }
...
data class Preferences(val mode: Mode = Mode.FRIENDLY, val chatGptEnabled: Boolean = true,
    val geminiEnabled: Boolean = true, val openAiModel: String = "gpt-4.1-mini",
...
object Prompts {
    fun forParticipant(speaker: Speaker, mode: Mode): String {
        val other = if (speaker == Speaker.CHATGPT) "Gemini" else "ChatGPT"
        ...
        else if (speaker == Speaker.CHATGPT)
            "You are ChatGPT in a group chat with Gemini and a human user. Be witty and casual. You can playfully tease Gemini if it makes a mistake or gives a vague answer, but stay likable."
        else "You are Gemini in a group chat with ChatGPT and a human user. Be witty and casual. You can playfully roast ChatGPT if its answer seems generic or overly cautious."
